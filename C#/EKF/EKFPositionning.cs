...
namespace EKF
{
    public class EKFPositionning
    {
        ...
        private readonly double[,] MatrixR;
        private readonly double[,] MatrixQ;
        private double[,] MatrixFx;
        private double[,] MatrixZPred;
        private double[,] MatrixZ;
        private readonly double[,] MatrixG;
        private readonly double[,] MatrixA;
        private readonly double[,] MatrixlowH;
        private double[,] MatrixHi;
        private readonly double[,] MatrixH;
        ...
        public double[,] Ajout_ld_P(double[,] P)
        {
            ...
            for (int row = 0; row < P.Length; row++)
            {
                ...
                {
                    matrice_apres[row, column] = matrice_apres[row, column];
                }
            }
            matrice_apres[P.Length, P.Length] = 30;                                                                     //Initialisation de P à "l'infini"
            matrice_apres[P.Length + 1, P.Length + 1] = 30;
        ...
        public List<int> acceuil_landmarks(List<List<double>> list_ld_recus, int grande_taille)   
        {
            ...
            for (int landmark = 0; landmark < list_ld_recus.Count; landmark++)
            {
                ...
                while ((!ld_identifié) & (4 + 2 * indice_en_cours < grande_taille))
                {
                ...
                if (!ld_identifié)
                {
                    ...
                    list_index.Add((grande_taille - 3) / 2);
                }
                ld_identifié = false;
            }
            return list_index;
        ...
        public void InitEKF(double xRefTerrain, double vxRefTerrain, double yRefTerrain, double vyRefTerrain, double theta, double vtheta, int id, double freqEchOdometry)
        {                                                                                                       // Ici on doit initialiser MatrixDelta, R et Q et les trucs qui ne changeront pas 
            int robotId = id;
            double fEch = freqEchOdometry;
            double tEch = 1 / freqEchOdometry;

            double[,] MatrixR = new double[5, 5];     
            double[,] MatrixZ = new double[2, 1];
            double[,] MatrixZPred = new double[2, 1];
            double[,] MatrixQ = new double[2, 2];
            double[,] MatrixParenthèses = new double[2, 2];
            double[,] MatrixPi = new double[5, 5];
            ...
            double[,] MatrixDelta = new double[2, 1];

            double[,] MatrixH = new double[2, 5];
            double[,] K = new double[5, 2];

            double[,] MatrixFx = new double[5, 5];
            double[,] MatrixG = new double[5, 5];                                                               //on fait ici l'identité de G zt il ne restera plus que les données odo à prendre 
        ...
        public void Trouver_est(List<int> List_indices, double [] X, double [,] P)     
        {
            ...
            {
                Matrixxest[indice, 0] = X[2 * item + 3];
                Matrixxest[indice+1, 0] = X[2 * item + 4];

                Matrixpest[0, indice    ] = P[0, 2 * item + 3]; //k
                Matrixpest[0, indice + 1] = P[0, 2 * item + 4]; //l
                Matrixpest[1, indice    ] = P[1, 2 * item + 3]; //m
                Matrixpest[1, indice + 1] = P[1, 2 * item + 4]; //n
                Matrixpest[2, indice    ] = P[2, 2 * item + 3]; //o
                Matrixpest[2, indice + 1] = P[2, 2 * item + 4]; //p
                Matrixpest[indice, 0] = P[2 * item + 3, 0];     //q
                Matrixpest[indice, 1] = P[2 * item + 3, 1];     //r
                Matrixpest[indice, 2] = P[2 * item + 3, 2];     //s
                Matrixpest[indice + 1, 0] = P[2 * item + 4, 0]; //v
                Matrixpest[indice + 1, 1] = P[2 * item + 4, 1]; //w
                Matrixpest[indice + 1, 2] = P[2 * item + 4, 2]; //x

                Matrixpest[indice, indice] = P[2 * item + 3, 2 * item + 3]; //t
                Matrixpest[indice, indice + 1] = P[2 * item + 3, 2 * item + 4]; //u
                Matrixpest[indice + 1, indice] = P[2 * item + 4, 2 * item + 3]; //y
                Matrixpest[indice + 1, indice + 1] = P[2 * item + 4, 2 * item + 4]; //z
        ...
        public void SLAMCorrection(double GPS_Theta, double Odo_VX, double Odo_VY, double Odo_VTheta, int nbre_landmarks, List<List<double>> landmarks_observés, List<int> list_indices)
        {
            ...
            for (int j = 0; j < nbre_landmarks; j++)                                                                    //on parcours 1 par 1 les landmarks
            {
                ...
                MatrixlowH[0, 4] = (1 / Math.Sqrt(q)) * deltay;
                ...
                MatrixParentheses = Toolbox.Multiply(MatrixHi, Toolbox.Multiply(MatrixP, Toolbox.Transpose(MatrixHi)));
                ...
                MatrixKdeltaz = Toolbox.Addition_Matrices(MatrixxPred, MatrixKdeltaz);
        ...
        public void OnLandmarksReceived(object sender, PointDExtendedListArgs e)  
        {
            if (robotId == e.RobotId)
            {
                ...
                {
                    InitEKF(currentGpsXRefTerrain, currentOdoVxRefRobot, currentGpsYRefTerrain, currentOdoVyRefRobot, currentGpsTheta, currentOdoVtheta, id, freqEchOdometry);
                ...
                Trouver_est(list_indice_landmarks, MatrixX, MatrixP);
